...
def tes(act_str):
    ...
    act_split_timestamp = act_str.split("@")
    act = act_split_timestamp[1].split(",")
    if (re.search(r'\d+,\d+,left', act_split_timestamp[1])) != None:
        global lc
        lc += 1
        test_list.append(f"def test_left_click{lc}():\n")
        test_list.append(f"\ttime.sleep({act_split_timestamp[0]})\n")
        test_list.append(f"\tmouse.move({act[0]},{act[1]}, duration=0.2)\n")
        test_list.append(f"\tmouse.on_button(t, buttons=\"left\", types=\"down\")\n")
        test_list.append("\tmouse.click()\n")
        test_list.append(f"\tassert(t() ==({act[0]},{act[1]})) \n")
    elif (re.search(r'\d+,\d+,right', act_split_timestamp[1])) != None:
        global rc
        rc += 1
        test_list.append(f"def test_right_click{rc}():\n")
        test_list.append(f"\ttime.sleep({act_split_timestamp[0]})\n")
        test_list.append(f"\tmouse.move({act[0]},{act[1]}, duration=0.2)\n")
        test_list.append(f"\tmouse.on_button(t, buttons=\"left\", types=\"down\")\n")
        ...
        test_list.append(f"\ttime.sleep({act_split_timestamp[0]})\n")
        test_list.append(f"\tmouse.move({act[0]},{act[1]}, duration=0.2)\n")
        test_list.append(f"\taverage=make_screenshot({screenshot_no},{act[0]},{act[1]})\n")
        test_list.append(f"\tassert(average < 1000) \n")
    elif (re.search(r'stop', act_split_timestamp[1])) != None:
        stop()
